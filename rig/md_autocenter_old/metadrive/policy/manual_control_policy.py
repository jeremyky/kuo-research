    ...
    if controller_name == "keyboard":
        ...
        try:
            if controller_name in ["steering_wheel", "wheel"]:
                controller = SteeringWheelController(enable_autocenter=enable_autocenter)
                if controller.steering_wheel is None:
                    print("Warning: Steering wheel not found or failed to initialize")
                    return None
                return controller
            else:
    ...
    def __init__(self, obj, seed, enable_expert=True):
        ...
        if config["manual_control"]:
            ...
            if self.controller is None:
                print("Warning: Load Joystick or Steering Wheel Error! Fall back to keyboard control")
    ...
    def act(self, agent_id):
        ...
        if self.engine.global_config["manual_control"] and is_track_vehicle and not_in_native_bev:
            try:
                ...
                print(f"Error processing controller input: {str(e)}")
                action = [0, 0]
...
class TakeoverPolicy(EnvInputPolicy):
    ...
    # ^ old code ... doesn't work . Jeremy added code below
