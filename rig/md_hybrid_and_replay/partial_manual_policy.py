...
class HybridEgoCarPolicy(_HybridBase):
    ...
    def act(self, agent_id):
        ...
        # Belt & suspenders: while data_collection keeps the wheel locked (GHOST/PREP phases),
        # force steering=0 so the sim can't twitch even if controller leaks.
        # This should NEVER be True during manual takeover (data_collection sets it to False).
        if bool(gc.get("autocenter_locked", False)):
            expert_action[0] = 0.0
            # Debug: This shouldn't happen during takeover
            if self.takeover:
                logger.warning("[Policy] WARNING: autocenter_locked is True during takeover! Steering zeroed.")
...
class HybridEgoCarReplayPolicy(_HybridBase):
    ...
    def reset(self):
        cfg = get_global_config()
        self.manual_inputs = None  # Will be loaded via prepare_inputs()
        self._use_states = bool(cfg.get("replay_use_states", False))
        self.config_manual = None
        return super().reset()
    
    def prepare_inputs(self):
        """Load manual inputs from JSON file and convert to list of [steering, acceleration] pairs."""
        cfg = get_global_config()
        manual_inputs_path = cfg.get("manual_inputs", None)
        
        if not manual_inputs_path:
            logger.warning("[Replay] No manual_inputs path in config")
            return []
        ...
        try:
            ...
            # Build list of [steering, acceleration] pairs
            values = []
            accelerations = inputs.get('accelerations', [])
            steering_signals = inputs.get('steering_signals', [])
            
            for idx, acc in enumerate(accelerations):
                if idx < len(steering_signals):
                    values.append([steering_signals[idx], acc])
                else:
                    values.append([0.0, acc])
            
            logger.info(f"[Replay] Loaded {len(values)} manual input frames from {manual_inputs_path}")
            return values
            
        except Exception as e:
            logger.warning(f"[Replay] Could not load manual inputs from {manual_inputs_path}: {e}")
            return []
    ...
    def act(self, agent_id):
        ...
        if not self.manual_inputs:
            self.manual_inputs = self.prepare_inputs()
        ...
        # Set replay_control flag to True so UI shows correct message
        try:
        ...
        # Simple indexing: value_index = index - onset
        value_index = index - self.config_manual
        
        # Return the action from manual inputs
        if value_index < len(self.manual_inputs):
