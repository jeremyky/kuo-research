...
# Add this try/except right under existing imports
try:
...
def get_controller(controller_name, pygame_control, enable_autocenter=True):
    ...
    if controller_name == "keyboard":
        ...
        "xboxcontroller", "xbox", "gamepad", "joystick",
        "steering_wheel", "wheel",
        "enhanced_wheel", "enhanced", "enhancedwheel"
    ]:
        try:
            if controller_name in ["steering_wheel", "wheel"]:
                return SteeringWheelController(enable_autocenter=enable_autocenter)
            elif controller_name in ["enhanced_wheel", "enhanced", "enhancedwheel"]:
                if EnhancedSteeringWheelController is None:
                    raise ImportError("Enhanced wheel controller not available on PYTHONPATH.")
                return EnhancedSteeringWheelController(enable_autocenter=enable_autocenter)
            else:
