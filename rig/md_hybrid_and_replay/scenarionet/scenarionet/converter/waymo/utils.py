...
def filter_scenario(md_scenario):
    """
    Decide whether to keep this scenario.
    Accepts a MetaDrive-style scenario dict returned from convert_waymo_scenario().
    """
    # Example 1: Require more than 5 agents
    if len(md_scenario[SD.TRACKS]) <= 5:
        return False

    # Example 2: Require pedestrian in tracks_to_predict
    for entry in md_scenario[SD.METADATA]["tracks_to_predict"].values():
        if entry["object_type"] == "TYPE_PEDESTRIAN":
            return True

    return False

def filtered_convert_waymo_scenario(scenario, version):
    md_scenario = convert_waymo_scenario(scenario, version)
    if filter_scenario(md_scenario):
        return md_scenario
    else:
