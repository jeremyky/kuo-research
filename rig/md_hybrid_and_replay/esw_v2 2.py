...
# add initial idle creep to run every time / dont need a throttle input first
# add a faster return for autocenter based on HOW MUCH the wheel has turned, for example, bigger steering 
# wheel deflection is a faster snap back (i.e. u turn return speed should not be same as right turn.)s
...
class EnhancedSteeringWheelController(SteeringWheelController):
    ...
    class _Options:
        ...
        return_alpha_min_per_sec = .8 # gentle at slow speed
        return_alpha_max_per_sec = 6 # very fast at high speed
        return_alpha_curve = 1.3 # >1 is softer ramp at slow speeds
