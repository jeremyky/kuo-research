...
class TrajectoryNavigation(BaseNavigation):
    ...
    def update_localization(self, ego_vehicle):
        ...
        self._navi_info.fill(0.0)
        for k, ckpt in enumerate(ckpts[0:]):
            #print("ckpt:", ckpt)
            #print("ego position:",ego_vehicle.position)
            start = k * self.CHECK_POINT_INFO_DIM
            end = (k + 1) * self.CHECK_POINT_INFO_DIM
            self._navi_info[start:end] = self._get_info_for_checkpoint(ckpt, ego_vehicle)
    ...
    def _get_info_for_checkpoint(cls, checkpoint, ego_vehicle):
        ...
        #print("ckpt position",checkpoint)
        #print("ego position",ego_vehicle.position)
        ...
        #print("ckpt in two dim",ckpt_in_heading, ckpt_in_rhs)
        ...
        #print("in navi info",clip((ckpt_in_heading / cls.NAVI_POINT_DIST + 1) / 2, 0.0, 1.0))
        ...
        #print("in navi info",clip((ckpt_in_rhs / cls.NAVI_POINT_DIST + 1) / 2, 0.0, 1.0))
        #print("navigation_information",navi_information)
